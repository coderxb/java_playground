/**
 * JedisTest.java   2016年8月30日 下午6:42:18 by LIXUEBING 
 *
 * Copyright (c) 2010 - 2016 xuebing.Li. All rights reserved.
 * 
 */
package indi.coderxb.easyblog.jedis;

import indi.coderxb.easyblog.utils.RedisUtil;
...
public class RedisUtilTest {
    ...
    @Test
    public void testReadFileContentSetData() {
        //读取一个大文件 100多MB的数据，设置信息
        File file = new File("E:\\kcsapi-debug.log.2016-09-07");
        ...
        jedis.set("name", str.toString());
        // 执行结果：minxr
        //System.out.println(jedis.get("name" + count));
        StringBuffer rStr = new StringBuffer();
        rStr.append(jedis.get("name"));

        System.out.println(rStr.substring(rStr.length() - 500));
        System.out.println("waiting check redis ...");
        jedis.close();
    }

    /**
     * try Unexpected status
     * 
     * Exception in thread "main" redis.clients.jedis.exceptions.JedisConnectionException: Unexpected end of stream.
     */
    @Test
    public void testPreCloseRedisConnectionAfterSetData() {
        //sleep 30 seconds， wait redis client clear link.
        try {
            Thread.sleep(30000);
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
        jedis.set("name", "test data");
    ...
    public void testMultiTask() {
        for (int i = 0; i < 500; i++) {
            Thread t = new Thread(new Runnable(){
                ...
                @Override
                public void run() {
                    Integer count = teamId.incrementAndGet();
                    Jedis jedis = RedisUtil.getJedis();
                    // 向key-->name中放入了value-->minxr  
                    jedis.set("name" + count, "minxr");
                    // 执行结果：minxr
                    System.out.println(jedis.get("name" + count));
                    System.out.println("waiting check redis ..., count = " + count);
                    try {
                        Thread.sleep(1000);
                    } catch (InterruptedException e) {
                        e.printStackTrace();
